...
_G.Settings = {	
	...
	Main = {
		...
		["Select Material"] = "";
		["Auto Farm Select Material"] = false,
		["Teleport To"] = "",

		--[Auto Farm Near Mob]

		["Distance Auto Farm Near Mob"] = 1000, -- {Max : 5000} 
	...
	Configs = {
		["Increment"] = 1, -- Attack Increment
		["Sea Travel"] = false, -- Save Travel in Pivate Server
        ["BadGFX"] = false,
        ["Damage Aura"] = false,
		["Double Quest"] = false,
		["Instant TP"] = false,
		...
		--[Misc Configs]
		["Auto Haki"] = true,
		["Distance Form Position"] = 20, --{Max : 50}
		["Camera Shaker"] = false,
	...
	Stat = {
		...
		["Select Stats"] = {""}, --{Max Stats,Melee,Defense,Sword,Devil Fruit,Gun}
...
function LoadSettings()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("KeneiSave") then
			makefolder("KeneiSave")
		end
		if not isfolder("KeneiSave/Blox Fruits/") then
			makefolder("KeneiSave/Blox Fruits/")
		end
		if not isfile("KeneiSave/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
			writefile("KeneiSave/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(_G.Settings))
		else
			local Decode = game:GetService("HttpService"):JSONDecode(readfile("KeneiSave/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"))
...
function SaveSettings()
	if readfile and writefile and isfile and isfolder then
		if not isfile("KeneiSave/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json") then
			...
			writefile("KeneiSave/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", game:GetService("HttpService"):JSONEncode(Array))
		end
	else
		return warn("Status : Undetected Executor")
	end
...
if _G.Settings.Configs["BadGfX"] then
    BadGFX()
end
...
function BadGFX()
    --// THIS WAS NOT MADE BY ME!!, Someone send it to me and asked me if I could please add it to my Alt Control and I said yes
	...
	game:GetService("RunService").Stepped:Connect(function()
		pcall(function()
			...
			if _G.Settings.Main["Auto Farm Level"] or _G.Settings.Main["Auto New World"] or
				...
				_G.Settings.Main["Auto Buddy Swords"] or _G.Settings.Main["Auto Farm Boss Hallow"] or _G.Settings.Main["Auto Farm Near Mob"] or _G.Settings.Main["Auto Material Soul Guitar"] or _G.Settings.Main["Auto Quest Soul Guitar"] or YamaQuest2 or YamaQuest1 or Auto_Cursed_Dual_Katana or
				Tushita_Quest2 or Tushita_Quest1 or _G.Settings.Main["Auto Farm Select Material"] or teleporttop or AutoFarmChest or 
				--[For God Human]
...
task.spawn(function()
	while task.wait() do
	    if _G.Kaitun then
			_G.Settings.Configs["White Screen"] = true
			_G.Settings.Configs["Instant TP"] = true
			_G.Settings.Stat["Enabled Auto Stats"] = true
			_G.Settings.Stat["Select Stats"] = "Kaitan"
			_G.Settings.Stat["Enabled Auto Redeem Code"] = true
			_G.Settings.Stat["Select Level Redeem Code"] = 1
			_G.Settings.Main["Auto Bring Fruit"] = true
			_G.Settings.Main["Auto Random Fruit"] = true
			_G.Settings.Main["Auto Farm Level"] = true
			_G.Settings.Main["Fast Auto Farm Level"] = true
			_G.Settings.Main["Auto Second Sea"] = true
			_G.Settings.Main["Auto Third Sea"] = true
			_G.Settings.FightingStyle["Auto God Human"] = true
			_G.Settings.Main["Auto Rainbow Haki"] = true
			_G.Settings.Main["Auto Farm Level"] = true
		end
	end
end)
...
task.spawn(function()
	while task.wait() do
		if _G.Settings.Main["Auto Random Fruit"] then
		...
		pcall(function()
			if StartMagnet then
				for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
					if not string.find(v.Name,"Boss") and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 290 then
...
function Have(Weaponname)
	for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventory")) do
		if type(v) == "table" then
			if v.Type == "Sword" then
				if v.Name == Weaponname then
					return true or 1 or yes
				end
			end
		end
	end
	return false or 0 or no
end
...
elseif placeId == 7449423635 then
	...
	game.Players.LocalPlayer:Kick("ERROR CODE : [DIRVW01A]\nIncorrect Map")
end
...
spawn(function()
	game:GetService('RunService').Stepped:Connect(function()
		if _G.Settings.Main["Auto Bring Fruit"] then
			...
			pcall(function()
				if _G.Settings.Main["Auto Farm Level"] then
					...
					if game:GetService("Players").LocalPlayer.Character:FindFirstChild("Leopard Fruit") or game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Leopard Fruit") then
						game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StoreFruit","Leo-Leo")
					end
				end
			end)
		end
...
spawn(function()
	pcall(function()
		while _G.Settings.Main["Auto Bring Fruit"] do task.wait(.1)
...
task.spawn(function()
	while task.wait() do
		Library:AttemptSave()
		Window:SetWindowTitle("Makima Kaitun - "..os.date('%A, %B %d %Y at %H:%M:%S'))
	end
end)
local Tabs = {
...
Main:AddToggle('AutoFarm', {
    Text = 'Start Kaitun';
    Defalut = _G.Settings.Main["Auto Farm Level"];
    Tolltip = 'Start Kaitun Farm';
}):OnChanged(function()
    _G.Settings.Main["Auto Farm Level"] = Toggles.AutoFarm.Value
    Auto_Farm_Level = Toggles.AutoFarm.Value
	Farming = Toggles.AutoFarm.Value
    _G.Settings.Main["Fast Auto Farm Level"] = Toggles.AutoFarm.Value
    SaveSettings()
end)

local Set = Tabs.General:AddLeftGroupbox('Settings')

coroutine.wrap(function()
	while task.wait() do
		if _G.Settings.Configs["Fast Attack"] == "Super Fast" then
		...
		local ac = CombatFrameworkR.activeController
		if ac and ac.equipped then
			if _G.Settings.Configs["Fast Attack"] or FastAttack then
				AttackFunction()
				if _G.Settings.Configs["Fast Attack Type"] == "Normal" then
					if tick() - cooldownfastattack > .9 then task.wait(.4) cooldownfastattack = tick() end
				elseif _G.Settings.Configs["Fast Attack Type"] == "Fast" then
				elseif _G.Settings.Configs["Fast Attack Type"] == "Slow" then
					if tick() - cooldownfastattack > .3 then task.wait(.6) cooldownfastattack = tick() end
                elseif _G.Settings.Configs["Fast Attack Type"] == "Super Slow" then
					if tick() - cooldownfastattack > .1 then task.wait(1) cooldownfastattack = tick() end
				end
...
Set:AddToggle('FT', {
    Text = 'Fast Teleport';
    Defalut = _G.Settings.Main["Instant TP"];
    Tolltip = 'Start Kaitun Farm';
}):OnChanged(function()
    _G.Settings.Main["Instant TP"] = Toggles.FT.Value
	SaveManager:Save('Kaitun')
end)
Set:AddToggle('White',{
	Text = 'White Screen',
	Defalut = _G.Settings.HUD["White Screen"],
	Tooltip = 'Boost your fps',
}):OnChanged(function()
	_G.Settings.HUD["White Screen"] = Toggles.White.Value
	SaveManager:Save('Kaitun')
	if Toggles.White.Value == true then
...
local AttackRandomType = 1
task.spawn(function()
	while task.wait() do 
		AttackRandomType = AttackRandomType+1
		if AttackRandomType >= 6 then AttackRandomType = 1 end
		task.wait(0.5)
	end
end)
local MyLevel = game.Players.LocalPlayer.Data.Level.Value
...
if MyLevel <= 5 then
	...
	Redeem('Sub2OfficailNobbie')
	Redeem('SUN2GAMERROBOT_EXP1')
	Redeem('THEGREATACE')
	Redeem('BIGNEWS')
	Redeem('FUDD10')
	Redeem('fudd10_v2')
	...
	Redeem('Sub2Fer999')
	Redeem('kittgaming')
	Redeem('SUB2GAMERROBOT_YT')
	Redeem('SUB2GAMERROBOT_RESET1')
	Redeem('SUB2UNKLEKIZARU')
	Redeem('SUB2GAMERROBOT_RESET')
end
...
function AutoFarmLevel()
	...
	if _G.Settings.Main["Fast Auto Farm Level"] and (MyLevel >= 30 and MyLevel <= 300) then
		if _G.Settings.Configs["Auto Haki"] then
			if not game.Players.LocalPlayer.Character:FindFirstChild("HasBuso") then
				game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Buso")
			end
		end
		if MyLevel >= 30 and MyLevel <= 90 then
		...
		elseif MyLevel >= 90 and MyLevel <= 300 then
					...
					if game:GetService("Workspace").Enemies:FindFirstChild("Shanda [Lv. 475]") then
						for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
							if _G.Settings.Main["Fast Auto Farm Level"] and v.Name == "Shanda [Lv. 475]" and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
								...
								until not (Farming and Auto_Farm_Level) or not (game:GetService("Workspace").Enemies:FindFirstChild("Shanda [Lv. 475]")) or not (_G.Settings.Main["Fast Auto Farm Level"] or _G.Settings.Main["Auto Farm Level"]) or v.Humanoid.Health <= 0 or not v.Parent
								FastAttack = false
							end
						end
					else
			...
			elseif GetQuest.Visible == true then
				...
				if table.find(AllPlayersTableSkipFarm,GetQuestTitle.Text:split(" ")[2]) then
					for i,v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
						if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text,v.Name) then
							if _G.Settings.Main["Auto Farm Level"] and _G.Settings.Main["Fast Auto Farm Level"] and v.Name == GetQuestTitle.Text:split(" ")[2] and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
								repeat task.wait()
									if Sea1 and (Vector3.new(61163.8515625, 11.6796875, 1819.7841796875) - v.HumanoidRootPart.Position).magnitude < 5000 then
										if FarmtoTarget then FarmtoTarget:Stop() end
									...
									elseif v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 150 then
										...
										if AttackRandomType == 1 then
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 14, 1)
										elseif AttackRandomType == 2 then
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 1, 14)
										elseif AttackRandomType == 3 then
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(1, 1, -14)
										elseif AttackRandomType == 4 then
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(14, 1, 0)
										elseif AttackRandomType == 5 then
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(-14, 1, 0)
										else
											game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0, 14, 1)
										...
										until _G.Settings.Main["Auto Farm Level"] or not (Farming or Auto_Farm_Level)
										game:service('VirtualInputManager'):SendKeyEvent(true, "X", false, game)
										game:service('VirtualInputManager'):SendKeyEvent(false, "X", false, game)
									end
								until not (Farming and Auto_Farm_Level) or not _G.Settings.Main["Auto Farm Level"] or not string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text,v.Name) or v.Humanoid.Health <= 0 or not v.Parent or GetQuest.Visible == false
							end
						end
					end
				else
					if game:GetService("Workspace").Enemies:FindFirstChild(Name) then
								...
								if string.find(GetQuestTitle.Text, QuestName) then
									repeat task.wait()
										if (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 150 then
											FarmtoTarget = toTarget(v.HumanoidRootPart.CFrame * CFrame.new(0,30,1))
										elseif v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 70 then
											if FarmtoTarget then FarmtoTarget:Stop() end
											StartMagnet = true
									...
									until not (Farming and Auto_Farm_Level) or not game:GetService("Workspace").Enemies:FindFirstChild(Name) or not _G.Settings.Main["Auto Farm Level"] or not string.find(GetQuestTitle.Text, QuestName) or v.Humanoid.Health <= 0 or not v.Parent or GetQuest.Visible == false
									FastAttack = false
									StartMagnet = false
								else
									Com("F_","AbandonQuest");
						...
						if Sea1 and (Name == "Fishman Commando [Lv. 400]" or Name == "Fishman Warrior [Lv. 375]") and (CFrameQuest.Position - game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart").Position).magnitude > 50000 then
							if Questtween then Questtween:Stop() end wait(.5)
							game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(61163.8515625, 11.6796875, 1819.7841796875))
						elseif Sea1 and not (Name == "Fishman Commando [Lv. 400]" or Name == "Fishman Warrior [Lv. 375]") and (CFrameQuest.Position - game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart").Position).magnitude > 50000 then
							if Questtween then Questtween:Stop() end wait(.5)
							game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(3864.8515625, 6.6796875, -1926.7841796875))
						elseif Sea2 and string.find(Name, "Ship") and (CFrameQuest.Position - game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart").Position).magnitude > 30000 then
							if Questtween then Questtween:Stop() end
							game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(923.21252441406, 126.9760055542, 32852.83203125))
						elseif Sea2 and not string.find(Name, "Ship") and (CFrameQuest.Position - game:GetService("Players").LocalPlayer.Character:WaitForChild("HumanoidRootPart").Position).magnitude > 30000 then
							if Questtween then Questtween:Stop() end
				...
				for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
					if _G.Settings.Main["Auto Farm Level"] and v.Name == Name and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
						if string.find(GetQuestTitle.Text, NameMon) then
							...
							until not (Farming and Auto_Farm_Level) or not game:GetService("Workspace").Enemies:FindFirstChild(Name) or not _G.Settings.Main["Auto Farm Level"] or not string.find(GetQuestTitle.Text, NameMon) or v.Humanoid.Health <= 0 or not v.Parent or GetQuest.Visible == false
							StartMagnet = false
							FastAttack = false
						else
							Com("F_","AbandonQuest");
...
function CheckNotifyComplete()
	for i, v in pairs(game:GetService("Players")["LocalPlayer"].PlayerGui:FindFirstChild("Notifications"):GetChildren()) do
		if v.Name == "NotificationTemplate" then
			if string.upper(v.Text):find("QUEST") then
				pcall(function()
					v:Destroy()
				end)
				return true
		...
		pcall(function()
			if _G.Settings.Main["Auto Farm Level"] then
												...
												if game.Workspace.Enemies:FindFirstChild("rip_indra [Lv. 1500] [Boss]") then 	
													for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
														if v.Name == "rip_indra [Lv. 1500] [Boss]" and v:FindFirstChild("Humanoid")and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
															...
															until not _G.Settings.Main["Auto Farm Level"] or not v.Parent or v.Humanoid.Health <= 0 
															tassk.wait(.5)
															Check = 2
															repeat task.wait() Com("F_","TravelZou") until Check == 1
															FastAttack = false
														end
													end
												else
													...
													for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
														if v.Name == "Swan Pirate [Lv. 775]" then
															pcall(function()
																...
																until not v.Parent or v.Humanoid.Health <= 0 or _G.Settings.Main["Auto Farm Level"] == false or game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == false
																FastAttack = false
																StartMagnet = false
											...
											if game.Workspace.Enemies:FindFirstChild("Jeremy [Lv. 850] [Boss]") then
												for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
													if v.Name == "Jeremy [Lv. 850] [Boss]" then
														...
														until not v.Parent or v.Humanoid.Health <= 0 or _G.Settings.Main["Auto Farm Level"] == false
														FastAttack = false
														if Farming == false then
															Farming = true
															Auto_Farm_Level = true
									...
									for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
										if v.Name == ("Darkbeard [Lv. 1000] [Raid Boss]" or v.Name == "Darkbeard [Lv. 1000] [Raid Boss]") and v.Humanoid.Health > 0 and v:IsA("Model") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
											repeat task.wait()
												...
												v.Humanoid:ChangeState(11)
												toTarget(v.HumanoidRootPart.CFrame * CFrame.new(0,30,0))
											until _G.Settings.Main["Auto Farm Level"] == false or v.Humanoid.Health <= 0
											StartMagnet = false
											FastAttack = false
											if Farming == false then
												Auto_Farm_Level = true
							...
							if _G.Settings.Main["Auto Farm Level"] then
								if game.Players.LocalPlayer.Data.Level.Value >= 700 then
									...
									if game.Workspace.Map.Ice.Door.CanCollide == true and game.Workspace.Map.Ice.Door.Transparency == 0 then
										...
										EquipWeapon("Key")
										repeat task.wait() toTarget(CFrame.new(1347.7124, 37.3751602, -1325.6488)) 
										until (CFrame.new(1347.7124, 37.3751602, -1325.6488).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.Settings.Main["Auto Farm Level"]
										...
										if game:GetService("Workspace").Enemies:FindFirstChild("Ice Admiral [Lv. 700] [Boss]") then
											for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
												if v.Name == "Ice Admiral [Lv. 700] [Boss]" and v.Humanoid.Health > 0 then
													...
													until v.Humanoid.Health <= 0 or not v.Parent or Auto_Farm_Level == true
													game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
												end
											end
										else
				...
				task.spawn(function()
					while task.wait() do
						pcall(function()
							if game.Players.LocalPlayer.Data.Level.Value >= 200 and Have("Saber") == false then
								if Farming then
									Auto_Farm_Level = false
									Farming = false
								end
														...
														for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
															if v.Name == "Mob Leader [Lv. 120] [Boss]" then
																repeat task.wait()
																	...
																	v.HumanoidRootPart.CanCollide = false
																	v.Humanoid:ChangeState(11)
																until v.Humanoid.Health <= 0 or _G.Settings.Main["Auto Farm Level"] == false or Have("Saber")== true
															end
														end
													end
				...
				task.spawn(function()
					while task.wait() do
						pcall(function()
							if (MyLevel >= 300) and (Have("Pole (1st Form)") == false) then
								if game.ReplicatedStorage:FindFirstChild("Thunder God [Lv. 575] [Boss]") or game.Workspace.Enemies:FindFirstChild("Thunder God [Lv. 575] [Boss]") then
									...
									for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
										if v.Name == "Thunder God [Lv. 575] [Boss]" and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
											...
											until not _G.Settings.Main["Auto Farm Level"] or v.Humanoid.Health <= 0 or not v.Parent or Have("Pole (1st Form)")  == true
											StartMagnet = false
											FastAttack = false
										end
									end
...
local Quests = Tabs.General:AddLeftGroupbox('Quests')

--[[✅]]

local Combat = Tabs.General:AddLeftGroupbox('Fighting Styles')
local DarkS = Combat:AddLabel('Dark Step                    : ❌')
local Elec = Combat:AddLabel('Electro                      : ❌')
local WaterK = Combat:AddLabel('Water Kung Fu                : ❌')
local DClaw = Combat:AddLabel('Dragon Claw                  : ❌')
local Sup = Combat:AddLabel('Superhuman                   : ❌')
local DeathS = Combat:AddLabel('Death Step                   : ❌')
local SharkM = Combat:AddLabel('Sharkman Karate              : ❌')
local EClaw = Combat:AddLabel('Electric Claw                : ❌')
local DTalon = Combat:AddLabel('Dragon Talon                 : ❌')
local Godh = Combat:AddLabel('Godhuman                     : ❌')

spawn(function()
    while task.wait() do
        if GetFightingStyle == 'Dark Step' then
            DarkS:SetText('Dark Step                    : ✅')
        end
        if GetFightingStyle == 'Electro' then
            Elec:SetText('Electro                       : ✅')
        end
        if GetFightingStyle == 'Water Kung Fu' then
            WaterK:SetText('Water Kung Fu               : ✅')
        end
        if GetFightingStyle == 'Dragon Claw' then
            DClaw:SetText('Dragon Claw                  : ✅')
        end
        if GetFightingStyle == 'Superhuman' then
            Sup:SetText('Superhuman                     : ✅')
        end
        if GetFightingStyle == 'Death Step' then
            DeathS:SetText('Death Step                   : ✅')
        end
        if GetFightingStyle == 'Sharkman Karate' then
            SharkM:SetText('Sharkman Karate              : ✅')
        end
        if GetFightingStyle == 'Elcetric Claw' then
            EClaw:SetText('Electric Claw                : ✅')
        end
        if GetFightingStyle == 'Dragon Talon' then
            DTalon:SetText('Dragon Talon                 : ✅')
        end
        if GetFightingStyle == 'Godhuman' then
            Godh:SetText('Godhuman                     : ✅')
        end
    end
end)

local Swords = Tabs.General:AddLeftGroupbox('Swords')
Swords:AddLabel('Cursed Dual Katana           : ❌')
Swords:AddLabel('True Triple Katana           : ❌')
Swords:AddLabel('Tushita                      : ❌')
Swords:AddLabel('Yama                         : ❌')
Swords:AddLabel('Spikey Trident               : ❌')
Swords:AddLabel('Twin Hook                    : ❌')
Swords:AddLabel('Cavender                     : ❌')
Swords:AddLabel('Buddy Sword                  : ❌')
Swords:AddLabel('Hallow Scythe                : ❌')
Swords:AddLabel('Shisui                       : ❌')
Swords:AddLabel('Saddi                        : ❌')
Swords:AddLabel('Wando                        : ❌')
Swords:AddLabel('Soul Cane                    : ❌')
Swords:AddLabel('Long Swords                  : ❌')
Swords:AddLabel('Gravity Cane                 : ❌')
Swords:AddLabel('Dragon Trident               : ❌')

local Gun = Tabs.General:AddLeftGroupbox('Guns')
Gun:AddLabel('Soul Guitar                  : ❌')
Gun:AddLabel('Serpent Bow                  : ❌')
Gun:AddLabel('Acuduim Rifle                : ❌')
Gun:AddLabel('Bizzare Rifle                : ❌')
Gun:AddLabel('Bazooka                      : ❌')
Gun:AddLabel('Kabucha                      : ❌')
Gun:AddLabel('Cannon                       : ❌')

local Asset = Tabs.General:AddLeftGroupbox('Assetsorys')
Asset:AddLabel('Pale Scraf                   : ❌')
Asset:AddLabel('Pliot Helmet                 : ❌')
Asset:AddLabel('Lei                          : ❌')
Asset:AddLabel('Hunter Cape (Red)            : ❌')
Asset:AddLabel('Hunter Cape (Green)          : ❌')
Asset:AddLabel('Hunter Cape (Black)          : ❌')
Asset:AddLabel('Zebra Cap                    : ❌')
Asset:AddLabel('Black Spikey Coat            : ❌')
Asset:AddLabel('Black Coat                   : ❌')

local Material = Tabs.General:AddLeftGroupbox('Materials')
local Mag = Material:AddLabel('Magma Ore                    : 00')
local Scp = Material:AddLabel('Scrap Metal                  : 00')
local Sky = Material:AddLabel('Sky Wings                    : 00')
local Fit = Material:AddLabel('Fish Tail                    : 00')
local Vam = Material:AddLabel('Vampire Fang                 : 00')
local Lat = Material:AddLabel('Lather                       : 00')
local Yeet = Material:AddLabel('Yeti Fur                     : 00')
local Dem = Material:AddLabel('Demonic Wisp                 : 00')
local Bon = Material:AddLabel('Bone                         : 00')
local Rad = Material:AddLabel('Radioactive Ore              : 00')
local Met = Material:AddLabel('Meteorite                    : 00')
local Mys = Material:AddLabel('Mystic Droplet               : 00')
local Min = Material:AddLabel('Mini Tusk                    : 00')
local Gun = Material:AddLabel('Gunpowder                    : 00')
local Dra = Material:AddLabel('Dragon Scale                 : 00')
local Con = Material:AddLabel('Conjured Cocoa               : 00')
local Alc = Material:AddLabel('Alchard Fragment             : 00')
local Dar = Material:AddLabel('Dark Fragment                : 00')
local Mir = Material:AddLabel('Mirror Fragtal               : 00')
			...
			if type(v) == "table" then
				if v.Type == "Material" then
					if v.Name == "Magma Ore" then
						Mag:SetText('Magma Ores                   : '..v.Count)
					end
					if v.Name == "Scrap Metal" then
						Scp:SetText('Scrap Metals                 : '..v.Count)
					end
					if v.Name == "Angel Wings" then
						Sky:SetText('Angel Wings                  : '..v.Count)
					end
					if v.Name == "Fish Tail" then
						Fit:SetText('Fish Tails                   : '..v.Count)
					end
					if v.Name == "Vampire Fang" then
						Vam:SetText('Vampire Fangs                : '..v.Count)
					end
					if v.Name == "Leather" then
						Lat:SetText('Leathers                     : '..v.Count)
					end
					if v.Name == "Yeti Fur" then
						Yeet:SetText('Yeti Fur                     : '..v.Count)
					end
					if v.Name == "Demonic Wisp" then
						Dem:SetText('Demonic Wisp                 : '..v.Count)
					end
					if v.Name == "Bone" then
						Bon:SetText('Bones                        : '..v.Count)
					end
					if v.Name == "Radioactive Material" then
						Rad:SetText('Radioactive Ore              : '..v.Count)
					end
					if v.Name == "Meteorite" then
						Met:SetText('Meteorites                   : '..v.Count)
					end
					if v.Name == "Mystic Droplet" then
						Mys:SetText('Mystic Droplet               : '..v.Count)
					end
					if v.Name == "Mini Tusk" then
						Min:SetText('Mini Tusks                   : '..v.Count)
					end
					if v.Name == "Gunpowder" then
						Gun:SetText('Gunpowders                   : '..v.Count)
					end
					if v.Name == "Dragon Scale" then
						Dra:SetText('Dragon Scales                : '..v.Count)
					end
					if v.Name == "Conjured Cocoa" then
						Con:SetText('Conjured Cocoa               : '..v.Count)
					end
					if v.Name == "Alchard Fragment" then
						Alc:SetText('Alchard Fragments            : '..v.Count)
					end
					if v.Name == "Dark Fragment" then
						Dar:SetText('Dark Fragments               : '..v.Count)
					end
					if v.Name == "Mirror Fragtal" then
						Mir:SetText('Mirror Fragtals              : '..v.Count)
					end
				end
			end
		end
...
--Right
local Client = Tabs.General:AddRightGroupbox('Client')
local Qn = Client:AddLabel('Quest : Null                    ')
local Ql = Client:AddLabel('QuestLevel : Null            : 0')
local Dd = Client:AddLabel('Doing Damage                 : 0')
local Dt = Client:AddLabel('Damage Total                 : 0')
local Ht = Client:AddLabel('Hitted Time                  : 0')
local Do = Client:AddLabel('Doing Tasks : Null')
Client:AddDivider()
local Fp = Client:AddLabel('000 : Frame Rate Per Seconds')
Client:AddLabel('Server Joined at')
local Dototal = 0
local Fps = math.huge
spawn(function()
	while task.wait() do		
		local DoingDmg = game:GetService("Players").LocalPlayer.PlayerGui.Main.DmgCounter.Text.Text
		Dototal = Dototal + DoingDmg
		CheckQuest()
		Qn:SetText('Quest : '..QuestName)
		Ql:SetText('QuestLevel : '..LevelQuest)
		Dd:SetText('Doing Damage : '..DoingDmg)
		Dt:SetText('Damage Total : '..Dototal)
		Ht:SetText('Hitted Time : '..Hitted)
		Fp:SetText(Fps..'Frame Rate Per Seconds')
	end
end)


local Status = Tabs.General:AddRightGroupbox('Stats')